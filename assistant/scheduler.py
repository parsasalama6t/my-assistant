...
Fixed-text rows (custom, task_reminder, event_reminder) are sent verbatim and
never touch the Claude API; briefing/review rows ask ``render_briefing`` for
their text.
"""
...
FIXED_TEXT_KINDS = ("custom", "task_reminder", "event_reminder")
...
class Scheduler:
    ...
    def tick(self) -> TickReport:
        ...
        for row in self.store.due_schedules(to_utc_iso(now)):
            ...
            if lateness > self.grace:
                if row["repeat"] or row["kind"] in AUTO_KINDS:
    ...
    # ------------------------------------------------------ auto reminders
    def _collect_task_reminders(self, now: datetime) -> dict[str, tuple[str, str]]:
        """source -> (fire_at_utc_iso, text) for open tasks with a due time."""
        ...
        for task in self.store.list_tasks("open"):
            ...
            out[f"task:{task['id']}"] = (to_utc_iso(fire), f"Reminder: {task['title']} is due {when}.")
        return out
    ...
    def _collect_event_reminders(self, now: datetime) -> dict[str, tuple[str, str]]:
        ...
        for event in self.store.list_events(start=window_start, end=window_end, limit=200):
            ...
            out[f"event:{event['id']}"] = (to_utc_iso(fire), text)
        return out

    def _collect_google_reminders(self, now: datetime) -> dict[str, tuple[str, str]] | None:
        ...
        for event in events or []:
            ...
            out[f"gcal:{event['id']}"] = (to_utc_iso(fire), text)
        return out

    def _reconcile(
        self, prefix: str, kind: str, live: dict[str, tuple[str, str]], target: tuple[str, str]
    ) -> None:
        ...
        for source, (fire_iso, text) in live.items():
